/* eslint-disable @typescript-eslint/no-use-before-define */ var _platformArchTriples_PlatformName;
...
const nextVersion = "13.4.12";
const ArchName = arch();
const PlatformName = platform();
const triples = ((_platformArchTriples_PlatformName = platformArchTriples[PlatformName]) == null ? void 0 : _platformArchTriples_PlatformName[ArchName]) || [];
const infoLog = (...args)=>{
...
async function tryLoadWasmWithFallback(attempts, isCustomTurbopack) {
    ...
    try {
        ...
        let bindings = await loadWasm(pathToFileURL(wasmDirectory).href, isCustomTurbopack);
        // @ts-expect-error TODO: this event has a wrong type.
        eventSwcLoadFailure({
            wasm: "fallback",
            nativeBindingsErrorCode: lastNativeBindingsLoadErrorCode
   ...
   */ function subscribe(useBuffer, nativeFunction) {
    ...
    class ProjectImpl {
        ...
        entrypointsSubscribe() {
            const subscription = subscribe(false, async (callback)=>binding.projectEntrypointsSubscribe(await this._nativeProject, callback));
            return async function*() {
                    ...
                    for (const { pathname , ...nativeRoute } of entrypoints.routes){
                        ...
                        switch(nativeRoute.type){
                            ...
                            case "conflict":
                                ...
                                break;
                            default:
                                invariant(nativeRoute, ()=>`Unknown route type: ${nativeRoute.type}`);
                                break;
                        }
    ...
    class EndpointImpl {
        ...
        async changed() {
            const iter = subscribe(false, async (callback)=>binding.endpointChangedSubscribe(await this._nativeEndpoint, callback));
            await iter.next();
            return iter;
        }
    }
                ...
                turbo: {
                    startDev: (options)=>{
                        if (!isCustomTurbopack) {
                            ...
                            Log.warn("Trying to load custom turbopack bindings. Note this is internal testing purpose only, actual wasm fallback cannot load this bindings");
                            Log.warn(`Loading custom turbopack bindings from ${__INTERNAL_CUSTOM_TURBOPACK_BINDINGS}`);
                            const devOptions = {
                                ...options,
