        ...
        await span.traceChild("create-trace-assets").traceAsyncFn(async ()=>{
            ...
            const result = await nodeFileTrace([
                ...
                readFile: async (path)=>{
                    if (chunksToTrace.has(path)) {
                        var _assets_nodePath_relative_replace;
                        const source = (_assets_nodePath_relative_replace = assets[nodePath.relative(outputPath, path).replace(/\\/g, "/")]) == null ? void 0 : _assets_nodePath_relative_replace.source == null ? void 0 : _assets_nodePath_relative_replace.source();
            ...
            for (const [entrypoint, entryFiles] of entryFilesMap){
                ...
                if (entrypoint.name.startsWith("app/")) {
                    ...
                    const clientManifestsForPage = entrypoint.name.endsWith("/page") || entrypoint.name === "/not-found" || entrypoint.name === "/_not-found" ? nodePath.join(outputPath, "..", entrypoint.name.replace(/%5F/g, "_") + "_" + CLIENT_REFERENCE_MANIFEST + ".js") : null;
