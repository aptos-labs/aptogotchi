...
import { SSG_GET_INITIAL_PROPS_CONFLICT, SERVER_PROPS_GET_INIT_PROPS_CONFLICT, SERVER_PROPS_SSG_CONFLICT, MIDDLEWARE_FILENAME, INSTRUMENTATION_HOOK_FILENAME } from "../lib/constants";
...
export async function printTreeView(lists, pageInfos, { distPath , buildId , pagesDir , pageExtensions , buildManifest , appBuildManifest , middlewareManifest , useStatic404 , gzipSize =true  }) {
    var _middlewareManifest_middleware;
    ...
    const printFileTree = async ({ list , routerType  })=>{
        ...
        messages.push([
            ...
            "Size",
            "First Load JS"
        ].map((entry)=>chalk.underline(entry)));
        filterAndSortList(list).forEach((item, i, arr)=>{
            ...
            const symbol = item === "/_app" || item === "/_app.server" ? " " : (pageInfo == null ? void 0 : pageInfo.static) ? "○" : (pageInfo == null ? void 0 : pageInfo.isSsg) ? "●" : isEdgeRuntime(pageInfo == null ? void 0 : pageInfo.runtime) ? "ℇ" : "λ";
    ...
    console.log(textTable(messages, {
        align: [
            "l",
            "l",
            "r"
    ...
    console.log(textTable([
        ...
        usedSymbols.has("λ") && [
            "λ",
            "(Server)",
            `server-side renders at runtime (uses ${chalk.cyan("getInitialProps")} or ${chalk.cyan("getServerSideProps")})`
        ],
        usedSymbols.has("○") && [
            "○",
            "(Static)",
        ...
        usedSymbols.has("●") && [
            "●",
            "(SSG)",
            `automatically generated as static HTML + JSON (uses ${chalk.cyan("getStaticProps")})`
        ],
        usedSymbols.has("ISR") && [
            "",
            "(ISR)",
            `incremental static regeneration (uses revalidate in ${chalk.cyan("getStaticProps")})`
        ]
    ].filter((x)=>x), {
        align: [
            "l",
    ...
    toPrerender.forEach((entry)=>{
        ...
        if (typeof entry === "string") {
            ...
            _validParamKeys.forEach((validParamKey)=>{
                ...
                builtPage = builtPage.replace(replaced, repeat ? paramValue.map((segment)=>escapePathDelimiters(segment, true)).join("/") : escapePathDelimiters(paramValue, true)).replace(/(?!^)\/$/, "");
...
export const collectGenerateParams = async (segment, parentSegments = [], generateParams = [])=>{
    var _segment_, _segment_1, _segment__layout, _segment__layout_, _segment_2, _segment__page, _segment__page_, _segment_3;
    ...
    const result = {
        ...
        config,
        getStaticPaths: isClientComponent ? undefined : mod == null ? void 0 : mod.getStaticPaths,
        generateStaticParams: isClientComponent ? undefined : mod == null ? void 0 : mod.generateStaticParams
    };
    if (segment[0]) {
...
export async function buildAppStaticPaths({ page , distDir , configFileName , generateParams , isrFlushToDisk , incrementalCacheHandlerPath , requestHeaders , maxMemoryCacheSize , fetchCacheKeyPrefix , staticGenerationAsyncStorage , serverHooks  }) {
    patchFetch({
    ...
    return StaticGenerationAsyncStorageWrapper.wrap(staticGenerationAsyncStorage, {
        pathname: page,
        renderOpts: {
...
export async function isPageStatic({ page , distDir , configFileName , runtimeEnvConfig , httpAgentOptions , locales , defaultLocale , parentId , pageRuntime , edgeInfo , pageType , originalAppPath , isrFlushToDisk , maxMemoryCacheSize , incrementalCacheHandlerPath  }) {
...
export async function copyTracedFiles(dir, distDir, pageKeys, appPageKeys, tracingRoot, serverConfig, middlewareManifest, hasInstrumentationHook) {
    ...
    await fs.writeFile(serverOutputPath, `${moduleType ? `import http from 'http'
import path from 'path'
...
export function getSupportedBrowsers(dir, isDevelopment, config) {
    let browsers;
    try {
    ...
    // prior to Next.js 13
    return config.experimental.legacyBrowsers ? undefined : MODERN_BROWSERSLIST_TARGET;
}
