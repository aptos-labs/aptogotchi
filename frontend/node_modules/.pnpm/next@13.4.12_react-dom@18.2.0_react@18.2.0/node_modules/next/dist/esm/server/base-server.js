...
import { setLazyProp, getCookieParser, checkIsOnDemandRevalidate } from "./api-utils";
...
import { addRequestMeta, getRequestMeta, removeRequestMeta } from "./request-meta";
...
import { RSC, RSC_VARY_HEADER, FLIGHT_PARAMETERS, NEXT_RSC_UNION_QUERY } from "../client/components/app-router-headers";
...
import { isRouteMatch, parsedUrlQueryToParams } from "./future/route-matches/route-match";
...
export default class Server {
    ...
    async handleRequest(req, res, parsedUrl) {
        ...
        return getTracer().trace(BaseServerSpan.handleRequest, {
   ...
   */ async prepare() {
    ...
    async runImpl(req, res, parsedUrl) {
        ...
        if (!this.serverOptions.webServerConfig && !getRequestMeta(req, "_nextIncrementalCache")) {
            let protocol = "https:";
            try {
                const parsedFullUrl = new URL(getRequestMeta(req, "__NEXT_INIT_URL") || "/", "http://n");
    ...
    async renderToResponseWithComponentsImpl({ req , res , pathname , renderOpts: opts  }, { components , query  }) {
        ...
        if (isAppPath) {
            ...
            if (!this.renderOpts.dev && !isPreviewMode && isSSG && req.headers[RSC.toLowerCase()]) {
                if (!this.minimalMode) {
                    isDataReq = true;
                }
                // strip header so we generate HTML still
                if (!isEdgeRuntime(opts.runtime) || this.serverOptions.webServerConfig) {
                    for (const param of FLIGHT_PARAMETERS){
                        delete req.headers[param.toString().toLowerCase()];
                    }
                }
        ...
        const doRender = async ()=>{
            ...
            const match = getRequestMeta(req, "_nextMatch") ?? // If the match can't be found, rely on the loaded route module. This
            ...
            ((this.renderOpts.dev || process.env.NEXT_RUNTIME === "edge") && components.routeModule ? {
                ...
                params: opts.params ? parsedUrlQueryToParams(opts.params) : undefined
            } : undefined);
            if (match && isRouteMatch(match, RouteKind.APP_ROUTE)) {
                const context = {
                    params: match.params,
            ...
            } else if (match && isRouteMatch(match, RouteKind.PAGES) && components.routeModule) {
            ...
            } else if (match && isRouteMatch(match, RouteKind.APP_PAGE) && components.routeModule) {
                const module = components.routeModule;
        ...
        if (!cachedData) {
            ...
            if (isAppPath) {
                ...
                return {
                    ...
                    body: isDataReq ? RenderResult.fromStatic(cachedData.pageData) : cachedData.html,
                    revalidateOptions
                };
            }
            return {
                type: isDataReq ? "json" : "html",
                body: isDataReq ? RenderResult.fromStatic(JSON.stringify(cachedData.pageData)) : cachedData.html,
                revalidateOptions
            };
        }
    }
