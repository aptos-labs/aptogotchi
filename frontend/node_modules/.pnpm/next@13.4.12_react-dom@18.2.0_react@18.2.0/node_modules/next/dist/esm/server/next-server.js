        ...
        if (this.isRouterWorker && !process.env.NEXT_MINIMAL) {
            ...
            this.renderWorkersPromises = new Promise(async (resolveWorkers)=>{
                try {
                    ...
                    if (this.hasAppDir) {
                        this.renderWorkers.app = await createWorker(ipcPort, ipcValidationKey, options.isNodeDebugging, "app", this.nextConfig);
                    }
                    this.renderWorkers.pages = await createWorker(ipcPort, ipcValidationKey, options.isNodeDebugging, "pages", this.nextConfig);
        ...
        await new Promise((proxyResolve, proxyReject)=>{
            ...
            if (upgradeHead) {
                ...
                proxy.web(req.originalRequest, res.originalResponse, {
                    buffer: (_getRequestMeta = getRequestMeta(req, "__NEXT_CLONABLE_BODY")) == null ? void 0 : _getRequestMeta.cloneBodyStream()
                });
            }
        });
        return {
        ...
        const catchAllRoute = {
            ...
            fn: async (req, res, _params, parsedUrl)=>{
                ...
                if (this.isRouterWorker) {
                    ...
                    (match == null ? void 0 : match.definition.kind) === RouteKind.APP_PAGE ? "app" : "pages";
                    ...
                    if (renderWorker) {
                        ...
                        for (const [key, value] of Object.entries(filterReqHeaders({
                            ...invokeRes.headers
                        }))){
                            if (value !== undefined) {
                                if (key === "set-cookie") {
            ...
            if (this.renderOpts.dev) {
                ...
                const reqCallback = ()=>{
                    ...
                    if (Array.isArray(fetchMetrics) && fetchMetrics.length) {
                        ...
                        process.stdout.write(`-  ${chalk.grey("┌")} ${chalk.cyan(req.method || "GET")} ${req.url} ${res.statusCode} in ${getDurationStr(reqDuration)}\n`);
   ...
   */ async runMiddleware(params) {
        ...
        const result = await run({
            ...
            request: {
                ...
                signal: signalFromNodeRequest(params.request.originalRequest)
            },
            useCache: true,
    ...
    getRoutesManifest() {
        return getTracer().trace(NextNodeServerSpan.getRoutesManifest, ()=>loadManifest(join(this.distDir, ROUTES_MANIFEST)));
    }
    ...
    async runEdgeFunction(params) {
        ...
        const result = await run({
            ...
            request: {
                ...
                signal: signalFromNodeRequest(params.req.originalRequest)
            },
            useCache: true,
            onWarning: params.onWarning,
