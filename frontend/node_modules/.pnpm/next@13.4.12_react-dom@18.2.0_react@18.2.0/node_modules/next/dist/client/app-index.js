...
function Root(param) {
    let { children  } = param;
    const [hadRuntimeError, setHadRuntimeError] = _react.default.useState(false);
    ...
    if (process.env.NODE_ENV !== "production") {
        ...
        const useWebsocket = require("./components/react-dev-overlay/internal/helpers/use-websocket").useWebsocket;
        // subscribe to hmr only if an error was captured, so that we don't have two hmr websockets active
        // eslint-disable-next-line react-hooks/rules-of-hooks
        const webSocketRef = useWebsocket(process.env.__NEXT_ASSET_PREFIX || "", hadRuntimeError);
        // eslint-disable-next-line react-hooks/rules-of-hooks
        _react.default.useEffect(()=>{
            const handler = (event)=>{
                let obj;
